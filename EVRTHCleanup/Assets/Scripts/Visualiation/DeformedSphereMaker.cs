...
namespace Visualiation
{
    public class DeformedSphereMaker : DataVisualizer
    {
        ...
        private Mesh GetFreshGlobeMesh()
        {
            ...
            {
                Ellipsoid scaledEllipsoid = new Ellipsoid(maxEarthRadius * globeEllipsoidType.Radii.x, maxEarthRadius * globeEllipsoidType.Radii.y, maxEarthRadius * globeEllipsoidType.Radii.z);
                mesh = tessellator.GenerateGlobe(scaledEllipsoid, StackPartitions, SlicePartitions);
            }
            else
            {
                Ellipsoid scaledEllipsoid = new Ellipsoid(minEarthRadius * globeEllipsoidType.Radii.x, minEarthRadius * globeEllipsoidType.Radii.y, minEarthRadius * globeEllipsoidType.Radii.z);
                mesh = tessellator.GenerateGlobe(scaledEllipsoid, StackPartitions, SlicePartitions);
            }
        ...
        public override IEnumerator DeformBasedOnNewTile(Tile tile,float min, float max)
        {
            ...
            VertexAndUvContainer vertexAndUvContainer = tessellator.LatLongBoundingBoxToGlobeVertexListAndUvs(tile.bBox, StackPartitions, SlicePartitions);
        ...
        private float ColormappedPixelColorToScienceValue(Color pixelColor, float minHeight, float rStep, float gStep, float bStep)
        {
